...
describe('the formula form service', () => {
  ...
  beforeEach(() => {
    ...
    formDataMock = { append: jasmine.createSpy('append') };
  });
  ...
  it('calculates outputs', async done => {
    ...
    await sut.collect($form, formDataMock);
    ...
    expect(formDataMock.append.calls.count()).toEqual(3);
    expect(formDataMock.append.calls.argsFor(0)).toEqual(['a', 1]);
    expect(formDataMock.append.calls.argsFor(1)).toEqual(['b', 2]);
    expect(formDataMock.append.calls.argsFor(2)).toEqual(['c', 0]);
    done();
  });
